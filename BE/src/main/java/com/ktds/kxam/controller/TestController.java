...
public class TestController {
    ...
    public @ResponseBody CommonResult modifyTest(@RequestBody TestTimeReqDTO modifyTestDTO) throws Exception {
        testService.modifyStartEndTime(modifyTestDTO.getTid(), modifyTestDTO.getStartTime(),
                modifyTestDTO.getEndTime());
        return responseService.getSuccessResult();
    }

    @Operation(description = "시험 응시자 비밀번호 수정")
    @PutMapping("/validate-key")
    public @ResponseBody CommonResult modifyValidateKey(@RequestBody TestValidateKeyReqDTO modifyTestValidateKeyDTO)
            throws Exception {
    ...
    @GetMapping
    public @ResponseBody ListResult<TestResDTO> findTestAll() {
        List<TestResDTO> result = testService.findTestAll();
        return responseService.getListResult(result);
    }

    @Operation(description = "특정 시험 조회, 전체 응시생 숫자 포함")
    @GetMapping("/info")
    public @ResponseBody ListResult<TestResDTO> findTestAll2(@RequestParam("tid") Long tid) {
        List<TestResDTO> result = testService.findTestByTid(tid);
        return responseService.getListResult(result);
    }
    ...
    @GetMapping("/score")
    public @ResponseBody SingleResult<Integer> getScore(@RequestParam("sabun") String sabun,
